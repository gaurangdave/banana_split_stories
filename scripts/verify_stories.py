...
def verify_story_object(story_obj: dict):
    ...
    if (not is_valid_string_value(story_obj, "theme")):
        raise ValueError("Invalid theme sotry_object")
    ...
    for step in story_tree:
        if not all(is_valid_string_value(step, k) for k in ["id", "scene_description", "character_pose_description", "narration"]):
            raise ValueError(
                f"Step '{step.get('id')}' has missing or invalid string fields.")

        if not is_valid_bool_value(step, "is_ending"):
            raise ValueError(
                f"Step '{step.get('id')}' has invalid 'is_ending' flag.")

        choices = step.get("choices", [])
        if step["is_ending"]:
            if choices:
                raise ValueError(
                    f"Ending step '{step['id']}' must not have choices.")
            if not is_valid_string_value(step, "outcome"):
                raise ValueError(
                    f"Ending step '{step['id']}' is missing an 'outcome'.")
        else:  # Not an ending step
            if not choices:  # Must have at least one choice
                raise ValueError(
                    f"Non-ending step '{step['id']}' has no choices.")
            # Validate that all next_ids point to a real step
            if len(choices) < 2:  # Must have at least one choice
                raise ValueError(
                    f"Non-ending step '{step['id']}' has less than 2 choices.")
            for choice in choices:
                next_id = choice.get("next_id")
                if not next_id or next_id not in all_step_ids:
                    raise ValueError(
                        f"Step '{step['id']}' has a choice pointing to an invalid next_id: '{next_id}'.")


def verify_naration(story_obj:dict, story_dir: Path):
    story_tree = story_obj["story_tree"]
    # 1. Prepare Once: Create a set of all valid IDs for instant lookups.
    all_step_ids = {step.get("id") for step in story_tree}
    # verify narration files
    narration_file_names = [
        f"{step_id}_narration.mp3" for step_id in all_step_ids]
    narration_file_names.append("prologue_narration.mp3")
    for narration_file in narration_file_names:
        narrataion_file_path = Path(story_dir,narration_file)
        if not narrataion_file_path.exists():
            raise ValueError(f"missing {narration_file}")
...
def verify_stories(data_root: str) -> list[str]:
    ...
    print(emoji.emojize(
        f":file_folder: Verifying stories from {data_root}..."))
    ...
    for theme in os.listdir(data_root):
        ...
        for story in os.listdir(theme_dir):
            ...
            for story_file in os.listdir(story_dir):
                ...
                print(emoji.emojize(
                    f"      :eyes: Verifying {story_file}..."), end=" ")
                ...
                try:
                    ...
                    verify_story_object(story_obj=story_obj)
                    verify_naration(story_obj=story_obj,story_dir=story_dir)
