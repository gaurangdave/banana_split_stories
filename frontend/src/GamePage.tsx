...
const GameEndScreen: FC<GameEndScreenProps> = ({ outcome, message }) => (
    <div className="text-center">
        <h2 className={`font-newsreader text-4xl font-bold mb-4 ${outcome === 'success' ? 'text-green-400' : 'text-red-400'}`}>
...
const GamePage: FC = () => {
  ...
  useEffect(() => {
    const startGame = async () => {
      ...
      if(character.mode === 'selfie') {
        ...
        formData.append('personalities', JSON.stringify(character.personalities));
  ...
  const renderContent = () => {
      ...
      if (!currentStep) {
          return <p className="text-center text-red-500">Could not load game data. Please try again.</p>;
      }
      ...
      return (
        <div className="md:flex h-full">
            ...
            <div className="w-full md:w-2/5 p-6 md:p-8 flex flex-col justify-between">
                <div>
                    <h2 className="font-newsreader text-2xl md:text-3xl font-bold text-[#f2a20d] mb-4 border-b-2 border-[#493b22] pb-2">The Story So Far</h2>
                ...
                <div className="mt-8">
                    <h3 className="font-newsreader text-xl text-[#f2a20d] mb-4">Your Choice...</h3>
                    <div className="space-y-4">
                        {currentStep.choices.map((choice: any, index: number) => (
                            <ChoiceButton key={index} text={choice.text} onClick={() => handleChoice(index)} />
                        ))}
                    </div>
                </div>
            </div>
            ...
            <div className="w-full md:w-3/5 p-4 bg-black/30">
                <div className="aspect-[9/16] w-full max-w-md mx-auto h-full rounded-lg overflow-hidden shadow-lg shadow-black/50 border-2 border-stone-800/50">
                        <img src={currentStep.scene_image_url} alt="Current game scene" className="w-full h-full object-cover" />
                </div>
            </div>
        </div>
      )
